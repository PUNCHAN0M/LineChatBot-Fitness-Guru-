...
def compute_response(sentence):
    ...
    greeting_corpus = list(set(record['question'] for record in run_query('MATCH (n:Barista) RETURN n.question as question;')))
    ...
    if greeting_scores[max_greeting_score_index] > 0.8:
        ...
        my_cypher = f"MATCH (n:Barista) WHERE n.question = '{match_greeting}' RETURN n.msg_reply AS reply"
...
def linebot():
    ...
    try:
        ...
        if "ชื่อ" in msg and "อะไร" in msg:
            ...
            if user_name:
                line_bot_api.reply_message(tk, TextSendMessage(text=f"สวัสดีครับคุณ{user_name}"))
            else:
                line_bot_api.reply_message(tk, TextSendMessage(text="คุณชื่ออะไรครับ"))
            ...
            if name:
                ...
                line_bot_api.reply_message(tk, TextSendMessage(text="ไม่ทราบว่าคุณชื่ออะไรครับ"))
